...
> ⚠️ Bu ağaç 2026-07-20'de canlı Supabase şemasıyla (PostgREST OpenAPI spec)
...
│   │   score_coefficients.sql, tercih_robotu.sql, tercih_aralik.sql  ← §11: tercih robotu + puan hesaplama
...
│   ├── badges.sql, notifications.sql  ← rozet/başarım sistemi + bildirim gelen kutusu
│   │   (2026-07-16 eklendi, bkz. §6.8)
│   ├── deneme_weak_topics.sql  ← mock_exams.weak_topic_ids + finish_quiz()'e
│   │   'weak_topics' modu ekler (2026-07-16, bkz. §6.8) — finish_quiz.sql'in
│   │   ORİJİNALİ değil, onu SONRADAN değiştiren bir migration
│   ├── functions/streak-reminder/  ← günlük "serini kaybetme" push cron'u (§6.8)
│   └── README.md           ← Supabase kurulum checklist'i — ⚠️ 2026-07-14'ten beri
│                              elle güncellenmedi, badges/notifications/deneme_weak_topics
│                              satırları YOK; DOSYALARIN SIRASI/İÇERİĞİ hâlâ doğru,
│                              güncel liste için §10'daki kurulum sırasına bak.
├── atlas-mobile/           ← Expo SDK 57 uygulaması
│   ├── src/app/            ← expo-router ekranları (index=Ev, harita, koc, yanlislar, kale/, tabs,
│   │                          rozetler.tsx, bildirimler.tsx, deneme/quiz-hedef.tsx — §6.8)
│   ├── src/components/     ← app-tabs, badges/, hearts/, koc/, map/, payment/, premium/, score/, ui/
...
| `mock_exams` | deneme sonuçları | nets jsonb: {"turkce":32.5,...}, weak_topic_ids uuid[] (2026-07-16, bkz. §6.8) |
| `weekly_exams` | Pazar sınavları | unique(user,week_start), question_ids uuid[], notified_at |
| `badges` | rozet kataloğu (10 satır, seed'li) | key, threshold_type (streak/topics_done/total_xp/mistakes_resolved), threshold_value — bkz. §6.8 |
| `user_badges` | kullanıcının kazandığı rozetler | unique(user_id,badge_id), earned_at — KALICI, geri alınmaz |
| `notifications` | uygulama içi bildirim gelen kutusu | title, body, route, read_at — badges/streak-reminder yazar |
...
> Bu bölüm başlangıçta bir "yapılacaklar" listesiydi. §6.1-§6.6 ve §6.8'deki
> HEPSİ artık canlı Supabase projesinde mevcut ve doğrulandı (PostgREST şeması
> üzerinden: tüm RPC'ler — `finish_quiz`, `get_hearts`, `lose_heart`,
> `refill_hearts`, `check_and_award_badges` — ve tüm Edge Function'lar çalışır
> durumda). Gerçek açık işler için §6.7'ye bak. Aşağıdaki alt başlıklar artık
> spesifikasyon/referans olarak okunmalı, "yapılacak" olarak değil.
...
6. ~~Koç günlük proaktif bildirimleri~~ → **kısmen çözüldü (2026-07-16).**
   Genel "serini kaybetme" hatırlatması `streak-reminder` Edge Function'ıyla
   geldi (§6.8) — ama bu kişiselleştirilmiş bir "koç" mesajı değil, herkese
   aynı sabit metin. Deepseek destekli kişiselleştirilmiş proaktif koç mesajı
   hâlâ açık, v2'ye bırakıldı.
...
8. **Push bildirimleri prodüksiyonda çalışmıyor** — `atlas-mobile/app.json`'da
   `extra.eas.projectId` HENÜZ TANIMLI DEĞİL. Bu olmadan gerçek cihazlarda
   `expo_push_token` hiç alınamıyor (bkz. `src/lib/push-notifications.ts`,
   `supabase/functions/streak-reminder/index.ts` başlık yorumu). Yani
   `weekly-exam` ve `streak-reminder` cron'ları deploy edilse bile `eas init`
   ile proje bağlanana kadar kimseye push ulaşmaz — uygulama içi bildirim
   gelen kutusu (`notifications` tablosu, §6.8) push'tan bağımsız çalışır,
   etkilenmez.
9. **Koç sekmesi "Deneme Sonucu Gir" formunda Matematik alanı eksikti,
   2026-07-20'de eklendi** — TYT'nin en yüksek ağırlıklı dersi (40 soru)
   önceden hiç net girilemiyordu (`(tabs)/koc.tsx` `DENEME_FIELDS`). Canlı
   DB'deki mevcut `mock_exams` kayıtları bu boşluğu doğruluyordu (nets
   içinde hiç "Matematik" anahtarı yoktu). Geriye dönük kayıtlar etkilenmez,
   yeni girişlerden itibaren geçerli.

### 6.8 Engagement Paketi — TAMAMLANDI (2026-07-16, canlı DB'de doğrulandı 2026-07-20)

BACKEND.md'nin 2026-07-14 sürümünde hiç yer almayan, sonradan tek oturumda
eklenen bir grup özellik — hepsi canlı Supabase projesinde doğrulandı
(`badges`: 10 satır kataloğu; `notifications`: 2 satır; `mock_exams.
weak_topic_ids` kolonu mevcut).

- **Rozetler** (`supabase/badges.sql`) — 10 satırlık sabit katalog (seri 3/7/30
  gün, 1/5/20 konu fethi, 500/2000 XP, 10/50 yanlış temizleme). `user_badges`
  KALICI (rozet geri alınmaz). `check_and_award_badges()` RPC'si (security
  definer) eşiği geçilmiş-ama-kazanılmamış rozetleri kaydeder + `notifications`e
  satır düşer + yeni kazanılanları döner; Ev ekranı yüklenişinde ve HER quiz
  bitişinde (kale/haftalık/tekil/hedefli pratik — 5 quiz ekranının hepsi)
  çağrılır. İstemci: `rozetler.tsx` (tam liste), Ev ekranındaki teaser kartı,
  `badge-unlock-popup.tsx` (kutlama popup'ı, tüm quiz sonuç ekranlarında).
- **Uygulama içi bildirim gelen kutusu** (`supabase/notifications.sql`) —
  yazma yalnız SECURITY DEFINER RPC/service-role function üzerinden (istemci
  insert edemez). İstemci: `bildirimler.tsx`, Ev ekranındaki 🔔 ikonu + kırmızı
  nokta (`fetchUnreadNotificationCount`). **Push'tan bağımsız** — push
  ulaşmasa bile (bkz. §6.7 madde 8) bu gelen kutusu çalışır.
- **`streak-reminder` Edge Function** (günlük cron, önerilen `0 17 * * *` =
  20:00 TR) — bugün hiç `xp_events` satırı olmayan TÜM kullanıcılara (yalnız
  premium değil) "🔥 Serini Kaybetme!" bildirimi; aynı gün tekrar göndermemek
  için idempotent kontrol. **Henüz deploy/cron kurulumu doğrulanmadı** —
  dosya repoda var ama Dashboard'da schedule kurulu mu ayrıca kontrol edilmeli.
- **Deneme sonrası hedefli pratik quiz** (`supabase/deneme_weak_topics.sql`)
  — Koç sekmesinde deneme sonucu girerken kullanıcı artık gerçek konu ağacından
  (subjects→units→topics) "zorlandığım konular" seçebiliyor (`WeakTopicsPicker`
  bileşeni, `mock_exams.weak_topic_ids`). `finish_quiz()`e yeni bir mod
  (`'weak_topics'`) eklendi — **`finish_quiz.sql`'in orijinal içeriği DEĞİŞMEDİ,
  bu dosya onu SONRADAN `create or replace function` ile genişletiyor** (2
  satır fark: izin verilen mod listesi + "doğru cevapta yanlışı temizle" mod
  listesi). Konu ilerlemesine/kale fethine DOKUNMAZ (ad-hoc pekiştirme). Yeni
  ekran: `atlas-mobile/src/app/deneme/quiz-hedef.tsx` (kale quiz'inin faz
  makinesinin birebir kopyası).
- **Deneme Net Takibi artık kendi sekmesi** — önceden Puan sekmesinin bir
  parçasıyken, 07-16'da `(tabs)/deneme.tsx` adında bağımsız bir sekmeye
  taşındı (trend grafiği `components/score/net-trend-chart.tsx` — proje
  içinde grafik kütüphanesi YOK, `react-native-svg` ile elle çizildi).
- **Koç "Bu Hafta" özeti** (`fetchWeeklySummary`) — son 7 günün XP/quiz
  sayısı/temizlenen yanlış sayısı, Koç sekmesinin üst çip satırında.
- **Ev ekranından çalışma takvimi kaldırıldı, rozetler ayrı sayfaya taşındı**
  (07-16, UI sadeleştirme — çalışma takvimi hâlâ kodda yok, kaldırıldığı gibi
  kaldı, geri eklenmedi).
...
#    → tercih_aralik.sql → notifications.sql → badges.sql → deneme_weak_topics.sql
#    (2026-07-20 itibarıyla hepsi canlı projede zaten uygulanmış durumda —
#    bu sıra yalnız SIFIRDAN bir Supabase projesi kurulumu için geçerli.
#    notifications.sql badges.sql'den ÖNCE gelmeli — badges.sql ona satır yazıyor.
#    deneme_weak_topics.sql finish_quiz.sql'den SONRA gelmeli — onu replace ediyor.)
...
(en önemlisi: `seed_tarih_full.sql` çalıştırılmadı, `payments` tablosu boş/
iyzico uçtan uca test edilmedi, push bildirimleri `eas.projectId` eksikliği
yüzünden prodüksiyonda hiç ulaşmıyor).
...
**2026-07-20 yeniden doğrulama:** Aynı yöntemle (`Prefer: count=exact`,
salt-okunur) tüm tablolar tekrar sayıldı — içerik sayıları DEĞİŞMEMİŞ
(11 ders/12 ünite/40 konu/194 soru/101 flashcard, `units` başına Tarih 2/
diğerleri 1 → `seed_tarih_full.sql` hâlâ çalıştırılmadığı doğrulandı),
`payments` hâlâ 0 satır. Yeni doğrulanan: `badges` 10, `notifications` 2,
`mock_exams` 2 (`weak_topic_ids` kolonu mevcut), `coach_messages` 28,
`profiles` 5 — yani engagement paketi gerçek kullanıcı verisiyle test
edilmiş durumda (bkz. §6.8). `atlas-mobile/app.json` içinde `extra.eas.
projectId` aranıp bulunamadı (push bildirimi kısıtı, §6.7 madde 8).
